...
const PostUpVip = ({postType='all'}) => {
    ...
    const fetchPage = async (cursor = null, useTier = true) => {
        ...
        try {
            ...
            if (useTier && postType && postType === 'all') {
                params.postTier = postTier.join(',');
            }
            if (cursor) {
                params.after = cursor;
            }
            ...
            // Nếu đang lọc theo tier mà không có tin nào ở lần load đầu tiên,
            // fallback sang lấy tin mới nhất bình thường để không bị trống.
            if (useTier && !cursor && newItems.length === 0) {
